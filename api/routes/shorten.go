...
import (
	...
	"github.com/ikhwankhaliddd/shorten-url-gofiber-redis/helpers"
)
...
func ShortenURL(c *fiber.Ctx) error {
	...
	//TODO: check if the input is an actual URL or not

	if !govalidator.IsURL(body.URL) {
		return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{"error": "Invalid URL"})
	}

	//TODO: check for the domain error

	if !helpers.RemoveDomainError(body.URL) {
		return c.Status(fiber.StatusServiceUnavailable).JSON(fiber.Map{"error": "Domain Error"})
	}

	//TODO: enforce https, SSL certificates
